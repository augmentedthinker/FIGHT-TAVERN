...
function setupGraphics() {
    ...
    if (!container) return; // Guard clause in case script loads before DOM
...
function setupInteractions() {
    ...
    document.querySelectorAll('.color-option').forEach(opt => {
        opt.addEventListener('click', (e) => {
            ...
            if (mainBtn) mainBtn.style.background = cssCol;
            if (rollBtn) rollBtn.style.background = cssCol;
...
function showResult() {
    ...
    // --- CRITICAL CONNECTION TO GAME LOGIC ---
    if (window.handleDiceResult) window.handleDiceResult(result);
}

function animate() {
    // Only animate if the modal is active (Performance Optimization)
    // Note: The original code didn't check for this, but the user asked for optimization.
    // However, to keep strict 1:1 functionality request, I will run it always, 
    // but a check for document.getElementById('dice-modal-overlay').classList.contains('active') is wise.
    // Given the prompt "make it more concise", I will leave it running but check for existence.
